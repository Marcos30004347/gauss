...
Expr evaluateProduct(Expr &u, Expr &v) {
  if (isConstant(u) && isConstant(v)) {
    ...
    Expr f =
        fraction(evaluateProduct(num_u, num_v), evaluateProduct(den_u, den_v));
...
Expr evaluateAddition(Expr &u, Expr &v) {
  ...
  if (u.kind() == Kind::Fraction && v.kind() == Kind::Integer) {
    ...
    Expr f = fraction(evaluateAddition(num_u, k), denominator(u));
  ...
  if (v.kind() == Kind::Fraction && u.kind() == Kind::Integer) {
    ...
    Expr f = fraction(evaluateAddition(k, num_v), denominator(v));
  ...
  if (v.kind() == Kind::Fraction && u.kind() == Kind::Fraction) {
    ...
    Expr f = fraction(evaluateAddition(num_u_t_dun_v, num_v_t_dun_u),
...
Expr evaluateSubtraction(Expr &u, Expr &v) {
  ...
  if (u.kind() == Kind::Fraction && v.kind() == Kind::Integer) {
    ...
    Expr f = fraction(evaluateSubtraction(num_u, k), denominator(u));
  ...
  if (v.kind() == Kind::Fraction && u.kind() == Kind::Integer) {
    ...
    Expr f = fraction(evaluateSubtraction(k, num_v), denominator(v));
  ...
  if (v.kind() == Kind::Fraction && u.kind() == Kind::Fraction) {
    ...
    Expr f = fraction(evaluateSubtraction(num_u_t_dun_v, num_v_t_dun_u),
                      evaluateProduct(den_u, den_v));
...
Expr evaluateDivision(Expr &v, Expr &w) {
  ...
  Expr f =
      fraction(evaluateProduct(num_v, den_w), evaluateProduct(num_w, den_v));
...
Expr evaluatePower(Expr &v, Expr &w) {
  ...
  if (num_v.kind() == Kind::Integer && num_v.value() != 0) {
    if (n > 0) {
      ...
      Expr f = fraction(denominator(v), numerator(v));
      Expr g = reduceRNEAST(f);
      Expr k = -n;

      return evaluatePower(g, k);
    }
  }
...
Expr simplyfyQuotient(Expr &u, Expr &v) { return u.value() / v.value(); }
    ...
    if (n.value() % d.value() == 0) {
      ...
      if (d.value() > 0) {
        if (g.value() > 0) {
          ...
          Expr k = -1;
          Expr a = evaluateProduct(k, g);
          return fraction(simplyfyQuotient(n, a), simplyfyQuotient(d, a));
        }
      } else if (d.value() < 0) {
        Expr k = -1;
        Expr prodn = evaluateProduct(n, k);
        Expr prodd = evaluateProduct(d, k);
...
Expr reduceRationalNumberASTRec(Expr &u) {
  ...
  if (u.size() == 1) {
    ...
    if (v.kind() == Kind::Undefined)
      ...
      Expr k = 1;
      return evaluateProduct(k, v);
    } else
      return u;
  }

  if (u.kind() == Kind::Addition || u.kind() == Kind::Multiplication ||
      u.kind() == Kind::Subtraction || u.kind() == Kind::Division) {
    Expr v = reduceRationalNumberASTRec(u[0]);
    Expr w = reduceRationalNumberASTRec(u[1]);

    if (v.kind() == Kind::Undefined || w.kind() == Kind::Undefined)
      return undefined();
...
Expr reduceRNEAST(Expr &u) { return reduceRNEAST(std::forward<Expr>(u)); }
