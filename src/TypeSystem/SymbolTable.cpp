...
unsigned long long calculateCRC64(const unsigned char *data, unsigned long size) {
...
struct symbol_registry {
	char** str; // str data array
	ull** crc; // crc data array
	ull** tree; // tree array, tree[i][j] stores where string j start on str[i]

	size_t* crc_alloc; // total number of allocated ull on crc[i]
	size_t* crc_count; // total number or crc's on crc[i]

	size_t* str_alloc; // total number of bytes allocated in str[i] buffer
	size_t* str_total; // total number of bytes used in str[i] buffer
};


symbol_registry* symbol_registry_create() {
	symbol_registry* table = new symbol_registry();

	table->str  = new char*[SYMBOL_TABLE_TABLE_SIZE];
...
ull symbol_registry_set_entry(symbol_registry *table, const char *id) {
	...
	ull str_alloc = table->str_alloc[idx];

	if(str_alloc <= str_total + n) {
		char* old = table->str[idx];

		table->str[idx] = new char[max(str_total + 1024, str_total + n + 1)];
	...
	memcpy(&table->str[idx][str_total], id, n * sizeof(char));

	table->str[idx][str_total + n] = '\0';

	ull crc_count = table->crc_count[idx];
	ull crc_total = table->crc_alloc[idx];

	if(crc_total <= crc_count + 1) {
		ull* old_crc  = table->crc[idx];
		ull* old_tree = table->tree[idx];
	...
	table->crc[idx][crc_count]  = crc;

	table->crc_count[idx] = table->crc_count[idx] + 1;
