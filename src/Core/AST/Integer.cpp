...
Int::Int(long int val) { this->val = val; }
Int::Int(unsigned long long val) { this->val = val; }
Int::Int(unsigned int val) { this->val = val; }
Int::Int(std::string val) { this->val = atof(val.c_str()); }
Int::Int(int v) { this->val = v; }
std::string Int::to_string() {
	std::ostringstream s;
	s << std::setprecision(numOfDigits(this->val));
  s << this->val;
  return s.str();
}
Int abs(Int a) { return std::fabs(a.val); }
...
Int pow(Int base, Int exp) { return std::pow(base.val, exp.val); }

Int max(Int a, Int b) { return std::max(a.val, b.val); }

Int min(Int a, Int b) { return std::min(a.val, b.val); }

bool operator<(const unsigned int &a, const Int &v) { return a < v.val; }
Int operator*(const int &a, const Int &v) { return a * v.val; }
bool operator>(const unsigned int &a, const Int &v) { return a > v.val; }
bool operator<=(const unsigned int &a, const Int &v) { return a <= v.val; }
bool operator>=(const unsigned int &a, const Int &v) { return a >= v.val; }
...
Int Int::operator+(const Int &v) const
{
        if (sign == v.sign)
        {
                ...
                for (int i = 0, carry = 0; i < (int) std::max(a.size(),
v.a.size()) || carry; ++i)
                {
                        if (i == (int) res.a.size())
                        {
...
v.a[i] : 0); carry = res.a[i] < 0; if (carry) res.a[i] += 	Int::base;
                        }
...
void Int::operator*=(long long v)
{
        ...
        if(v > 	Int::base)
        {
                *this = *this * (v / 	Int::base) * 	Int::base + *this * (v %
Int::base); return ;
        }
...
std::vector<int> Int::convert_base(const std::vector<int> &a, int old_digits,
int new_digits) { std::vector<long long> p(std::max(old_digits, new_digits) +
1); p[0] = 1;
...
std::vector<long long> Int::karatsubaMultiply(const std::vector<long long> &a,
const std::vector<long long> &b) { int n = a.size();
