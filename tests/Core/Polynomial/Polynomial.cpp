...
void should_calculate_monomial_division() {
  ...
  Expr u = power(x, 3) + 3*power(x, 2)*y + 4*x*power(y, 2);
	...
	assert(monomialPolyDiv(u,v,L) == list({
				-6 + 3*x + -5*y,
...
void should_pseudo_divide_polynomials() {
	...
	assert(R[1] == 8*power(y, 2) + 12*x*power(y, 3) + -30*x*power(y, 4) + -20*power(x, 2)*power(y, 4));
}
...
void should_algebraic_expand_expressions() {
	...
	Expr u0 = (x*power(y + 1, fraction(3, 2)) + 1)*((x*power(y + 1, fraction(3, 2))) - 1);

	assert(algebraicExpand(u0) == -1 + power(x, 2) + 3*power(x, 2)*y + 3*power(x, 2)*power(y, 2) + power(x, 2)*power(y, 3));

	Expr u1 = power(x * power(y + 1, fraction(1, 2)) + 1, 4);

	assert(algebraicExpand(u1) ==
				 1 + 6*power(x, 2) + power(x, 4) + 6*power(x, 2)*y + 2*power(x, 4)*y + power(x, 4)*power(y, 2) + 4*x*power(1 + y, fraction(1, 2)) + 4*power(x, 3)*power(1 + y, fraction(3, 2)));
	...
	assert(algebraicExpand(u3) == power(x, 3) + 3*power(x, 2)*y + 3*x*power(y, 2) + power(y, 3) + 3*power(x, 2)*z + 6*x*y*z + 3*power(y, 2)*z + 3*x*power(z,2) + 3*y*power(z, 2) + power(z, 3));
	...
	assert(algebraicExpand(u5) == 49 + 56*x + 30*power(x, 2) + 8*power(x, 3) + power(x, 4));

	Expr u6 = (-32*power(z, 3) + 32*power(z, 4) + 48*power(z, 5) + -24*power(z, 6) + -48*power(z, 7) + -36*power(z, 8) + -40*power(z, 9) + -8*power(z, 10) + -8*power(z, 11)) / (4 * power(z, 2));

	assert(algebraicExpand(u6) == -8*z + 8*power(z, 2) + 12*power(z, 3) + -6*power(z, 4) + -12*power(z, 5) + -9*power(z, 6) + -10*power(z, 7) + -2*power(z, 8) + -2*power(z, 9));
}
...
void should_expand_main_operator() {
  ...
  Expr u0 = x*(2 + power(1 + x, 2));
	...
	assert(algebraicExpandRoot(u1) == power(x, 2) + 2*x*power(1 + x, 2) + power(1 + x, 4));
}
...
void should_get_polynomial_content()
{
	...
	Expr b = (power(y, 2) + 2*y + 1)*power(x, 2) + (2*power(y, 2) - 2)*x + (3*y + 3);
...
void should_monomial_base_expand_polynomials()
{
	...
	Expr u = power(a, 2)*b + 2*a*power(b, 2) + power(b, 3) + 2*a + 2*b + 3;
...
int main() {
	...
	should_get_coefficients();
	should_get_leading_coefficient();
